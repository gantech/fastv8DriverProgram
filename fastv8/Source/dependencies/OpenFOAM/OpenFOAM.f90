...
!   call ReadInputFiles( InitInp_AD%InputFile, OpFM%m, NumBl, ErrStat2, ErrMsg2 )
   call OpFM_CreateActForceNodesMesh( InitInp, p_FAST, u_AD14, u_AD, InitInp_AD, y_AD, OpFM, ErrStat2, ErrMsg2)
...
!----------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE OpFM_CreateActForceNodesMesh( InitInp, p_FAST, u_AD14, u_AD, InitInp_AD, y_AD, OpFM, ErrStat, ErrMsg )
!..................................................................................................................................
   ...
   CHARACTER(*),                    INTENT(  OUT)  :: ErrMsg      ! Error message if ErrStat /= ErrID_None
      ...
      ! local variables                   
   TYPE(MeshType) , DIMENSION(:), ALLOCATABLE      :: forceNodesMotions      !< point mesh for transferring velocity actuator nodes (AeroDyn) to force actuator nodes (needs translationDisp and orientation) [-]
   TYPE(MeshMapType) , DIMENSION(:), ALLOCATABLE   :: velocity_to_force_nodes_Motions      !< mapping data structure to convert position and orientation of velocity nodes to force nodes [-]
   REAL(ReKi), DIMENSION(:,:), ALLOCATABLE         :: forceNodePositions  ! new positions for the force actuator nodes
   INTEGER(IntKi)                                  :: NnodesForce ! number of force actuator nodes
   ...
   CHARACTER(*),   PARAMETER                       :: RoutineName = 'OpFM_CreateActForceNodesMesh'
   ...
   ! number of force nodes in the interface
   NnodesForce = 1 + NumBl * InitInp%NumActForcePtsBlade + InitInp%NumActForcePtsTower
   ...
   IF ( p_FAST%CompAero == Module_AD ) THEN ! AeroDyn 15 needs mapping of line2 meshes to point meshes
      if ( y_AD%TowerLoad%NNodes > 0 ) then
         ...
         ! Allocate space for mapping data structures
      ALLOCATE( forceNodesMotions(NMappings), &
                velocity_to_force_nodes_Motions(NMappings),STAT=ErrStat2)
      IF (ErrStat2 /= 0) THEN
      ...
      END IF
         ...
         ! create meshes to map:            
      ALLOCATE(forceNodePositions(3,InitInp%NumActForcePtsBlade)) ! Allocate space to create new positions
      DO k=1,NumBl
         call MeshCreate ( BlankMesh = forceNodesMotions(k)         &
                          ,IOS       = COMPONENT_INPUT             &
         ...
         call CalcForceActuatorPositions(InitInp%NumActForcePtsBlade, u_AD%BladeMotion(k)%position, forceNodePositions, errStat2, errMsg2)
         do j=1,InitInp%NumActForcePtsBlade
            call MeshPositionNode(forceNodesMotions(k), j, forceNodePositions(:,j), errStat2, errMsg2)
               call SetErrStat( errStat2, errMsg2, errStat, errMsg, RoutineName )
               
            call MeshConstructElement( forceNodesMotions(k), ELEMENT_POINT, errStat2, errMsg2, p1=j )
               call SetErrStat( errStat2, errMsg2, errStat, errMsg, RoutineName )
         end do !j
         
         call MeshCommit(forceNodesMotions(k), errStat2, errMsg2 )
            call SetErrStat( errStat2, errMsg2, errStat, errMsg, RoutineName )
            if (errStat >= AbortErrLev) return                     
      end do
      DEALLOCATE(forceNodePositions) ! Free space 

      ALLOCATE(forceNodePositions(3,InitInp%NumActForcePtsTower)) ! Allocate space to create new positions
      call CalcForceActuatorPositions(InitInp%NumActForcePtsTower, u_AD%TowerMotion%position, forceNodePositions, errStat2, errMsg2)
      if ( y_AD%TowerLoad%NNodes > 0 ) then
         k = NMappings
         call MeshCreate ( BlankMesh = forceNodesMotions(k)        &
                          ,IOS       = COMPONENT_INPUT             &
      ...
      ! create the mapping data structures:
      DO k=1,NumBl 
         call MeshMapCreate( u_AD%BladeMotion(k), forceNodesMotions(k), velocity_to_force_nodes_Motions(k),  ErrStat2, ErrMsg2 );
            call SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
      END DO
      
      do k=NumBl+1,NMappings
         if ( y_AD%TowerLoad%nnodes > 0 ) then ! we can have an input mesh on the tower without having an output mesh. 
            call MeshMapCreate( u_AD%TowerMotion, forceNodesMotions(k), velocity_to_force_nodes_Motions(k),  ErrStat2, ErrMsg2 );
               call SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )         
         end if
      ...
      DO K = 1,SIZE(u_AD%BladeMotion)
         ...
         ! mesh mapping from line2 mesh to point mesh
         u_AD%BladeMotion(k)%Orientation = 0.0_ReKi
         do i = 1,u_AD%BladeMotion(k)%Nnodes
            do j = 1,3
               u_AD%BladeMotion(k)%Orientation(j,j,i) = 1.0_ReKi
            end do
         end do
         call Transfer_Line2_to_Point( u_AD%BladeMotion(k), forceNodesMotions(k), velocity_to_force_nodes_Motions(k), ErrStat2, ErrMsg2 )         
            call SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
         u_AD%BladeMotion(k)%Orientation     = u_AD%BladeMotion(k)%RefOrientation
...
END SUBROUTINE OpFM_CreateActForceNodesMesh
!----------------------------------------------------------------------------------------------------------------------------------
