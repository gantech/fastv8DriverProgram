...
void FAST_cInterface::stepNoWrite() {
...
int FAST_cInterface::setGlobalInputs(int nTsGlob, bool dRun, bool sController, std::string sControlLib, bool rStart, double timeStart, int nTimeStepStart, double timeEnd, int nTimeStepEnd, double timeMax, double timeStep, int nCheckpoint, int nScOutputs, int nScInputs ) {
  ...
  nTurbinesGlob = nTsGlob;
    ...
    if (nTurbinesGlob > 0) {
      ...
      debug = true; //Set this to false for production runs
      tStart = timeStart;
      nt_global = 0;
      if (rStart) {
	simStart = fast::restartDriverInitFAST;
...
void FAST_cInterface::setTurbineData(int tid, std::string fastInpFile, std::string fastRstartFile, std::vector<double> tPos, int nActForcePtsBlade, int nActForcePtsTower) {
  ...
  TurbID.resize(1);
  TurbineBasePos.resize(1);
  FASTInputFileName.resize(1);
  CheckpointFileRoot.resize(1);
  numBlades.resize(1);
  numForcePtsBlade.resize(1);
  numForcePtsTwr.resize(1);
  numVelPtsBlade.resize(1);
  numVelPtsTwr.resize(1);
  forceNodeVel.resize(1);

  TurbID[0] = tid;
  FASTInputFileName[0] = fastInpFile ;
  CheckpointFileRoot[0] = fastRstartFile ;
  TurbineBasePos[0].resize(3);
  for(int i=0;i<3;i++) {
    TurbineBasePos[0][i] = tPos[i];
  }
...
void FAST_cInterface::setInputs(const fast::fastInputs & fi ) {
...
void FAST_cInterface::getHubShftDir(std::vector<double> & hubShftVec, int iTurbGlob) {
...
void FAST_cInterface::setVelocityForceNode(std::vector<double> & currentVelocity, int iNode, int iTurbGlob) {
...
void FAST_cInterface::readVelocityData(int nTimesteps) {
...
hid_t FAST_cInterface::openVelocityDataFile(bool createFile) {
...
herr_t FAST_cInterface::closeVelocityDataFile(int nt_global, hid_t velDataFile) {
...
void FAST_cInterface::writeVelocityData(hid_t h5File, int iTurb, int iTimestep, OpFM_InputType_t iData, OpFM_OutputType_t oData) {
...
void FAST_cInterface::applyVelocityData(int iPrestart, int iTurb, OpFM_OutputType_t cDriver_Output_to_FAST, std::vector<double> & velData) {
...
void FAST_cInterface::loadSuperController(const fast::fastInputs & fi) {
